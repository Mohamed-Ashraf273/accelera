...
class InputSchema:
    ...
    def validate(self, input_data):
        ...
        self._coerce_columns(df, errors)
        if errors:
            raise SchemaValidationError(errors)
        ...
        for feature in self.features:
            ...
            if "min" in feature or "max" in feature:
                range_kwargs = {"column": column}
                if "min" in feature:
                    range_kwargs["min_value"] = feature["min"]
                if "max" in feature:
                    range_kwargs["max_value"] = feature["max"]
                expectations.append(
            ...
            for expectation in expectations:
                ...
                if not result.success:
                    ...
                    errors.append(f"{column} failed {expectation_name}")
    ...
    def _coerce_columns(self, df, errors):
        for feature in self.features:
            column = feature["name"]
            ...
            if feature_type in {"number", "integer"}:
                ...
                if invalid.any():
                    errors.append(f"{column} must be {feature_type}")
                    df[column] = original
                    continue
                ...
                if invalid.any():
                    errors.append(f"{column} must contain boolean values")
                df[column] = df[column].replace(
                    {"true": True, "false": False, "1": True, "0": False}
                )

            else:
                errors.append(f"{column} has unsupported type {feature_type!r}")
