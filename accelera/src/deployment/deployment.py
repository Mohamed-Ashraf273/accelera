    ...
    if df is not None:
        ...
        for col in df.columns:
            if col != target_col:
                ...
                feat_type = "integer" if "int" in dtype else ("number" if "float" in dtype else "string")
                features.append({"name": col, "type": feat_type})
        config_data["schema"] = {"features": features}
    else:
        config_data.setdefault("schema", {"features": []})

    config_data.setdefault("tracking", {"enabled": True, "path": "prediction_logs/predictions.jsonl"})
