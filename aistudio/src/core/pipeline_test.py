...
def test_parallel_execution_performance():
    """Test that parallel execution works and provides performance benefits"""
    import time
    ...
    p_sequential = Pipeline()
    p_sequential.enable_parallel(False)
    p_sequential.branch(
        "models",
        p_sequential.model(
            "lr1",
        ...
        p_sequential.model(
            "lr2",
            LogisticRegression(random_state=43, max_iter=1000),
            branch=True,
        ),
        p_sequential.model(
            "lr3",
            LogisticRegression(random_state=44, max_iter=1000),
            branch=True,
        ),
        p_sequential.model(
            "lr4",
    ...
    p_sequential.predict("pred", test_data)
    p_sequential.merge("average", lambda preds: np.mean(preds, axis=0))

    start_time = time.time()
    result_sequential = p_sequential(X, y)
    sequential_time = time.time() - start_time

    p_parallel = Pipeline()
    p_parallel.enable_parallel(True)
    p_parallel.set_multicore_threshold(2)
    p_parallel.branch(
        "models",
        p_parallel.model(
            "lr1",
    ...
    p_parallel.predict("pred", test_data)
    p_parallel.merge("average", lambda preds: np.mean(preds, axis=0))

    start_time = time.time()
    result_parallel = p_parallel(X, y)
    parallel_time = time.time() - start_time

    assert len(result_sequential) == len(result_parallel)
    if len(result_sequential) > 0 and len(result_parallel) > 0:
        seq_pred = (
            result_sequential[0]
            if isinstance(result_sequential, list)
            else result_sequential
        )
        par_pred = (
            result_parallel[0]
            if isinstance(result_parallel, list)
            else result_parallel
        )
        assert np.allclose(seq_pred, par_pred, rtol=1e-6), (
    ...
    assert parallel_time > 0, "Parallel execution should complete successfully"
