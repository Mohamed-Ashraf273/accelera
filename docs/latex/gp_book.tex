...
For the Deployment module, the major challenges included:
\begin{itemize}[leftmargin=*]
  \item \textbf{Image size optimization}: Eliminating heavy training dependencies (such as PyTorch and TensorFlow) from the runtime serving container to minimize size and deployment time.
  \item \textbf{Robust type coercion}: Dealing with edge-case inputs (e.g. string representations of numeric and boolean fields) during runtime schema validation without throwing false positives.
  \item \textbf{Headless remote orchestration}: Programmatically resolving host authentication, SSH key security, and Docker daemon startup on remote AWS EC2 instances during fully automated deployments.
\end{itemize}

\section{Gained Experience}
Developing the Accelera system provided valuable hands-on experience in:
\begin{itemize}[leftmargin=*]
  \item Designing parallel Graph-based execution backends for machine learning pipelines.
  \item Building source-to-source compilers with loop analyzers, classification guide systems, and AST code generation.
  \item Constructing lightweight FastAPI-based model serving layers with Great Expectations validation pipelines.
  \item Implementing a custom VCS for machine learning registries to enable repeatable snapshots and rollbacks.
  \item Automating cloud orchestration pipelines (Docker, Heroku, AWS EC2) inside continuous deployment constraints.
\end{itemize}
...
The Deployment module successfully bridges the gap between training and production. It automates staging, runtime validation, telemetry logging, target containerizing, and historical version rollbacks. This ensures that machine learning models can be securely and repeatably served on local, Heroku, or AWS EC2 platforms.
...
For the Deployment module, future research directions include:
\begin{itemize}[leftmargin=*]
  \item \textbf{Serverless target support}: Introducing serverless handlers (such as AWS Lambda or Google Cloud Functions) to allow on-demand serving and minimize hosting costs.
  \item \textbf{Live monitoring and drift detection}: Integrating online data drift monitoring engines (e.g. evidently.ai) to alert users when inference data distribution shifts from training data.
  \item \textbf{Distributed VCS registries}: Extending the internal VCS to synchronize snapshots with centralized cloud storage (like AWS S3) or model registries (like MLflow).
\end{itemize}
