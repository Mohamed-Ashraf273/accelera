...
Simple, mature, and widely used linear workflow abstraction. &
\texttt{accelera\_pipe} adds a graph backend, branching, executed-graph reuse,
and integration with custom-code acceleration. \\

OMPar &
AI-assisted source-to-source OpenMP parallelization for C/C++ code. &
...
The graph avoids duplicated work when a split starts with equivalent branch nodes. 
For example, if the branches contain repeated preprocessing operations such as \texttt{MinMaxScaler}, 
the graph creates only one shared node for that operation. This sharing depends on the node type and object identity or 
representation, not only on the visible node name. However, sharing is limited to the first node of each branch path, 
because later nodes may receive different inputs from previous operations and therefore must 
remain separate. Figure~\ref{fig:branch-sharing} illustrates this branch-entry sharing behavior.
