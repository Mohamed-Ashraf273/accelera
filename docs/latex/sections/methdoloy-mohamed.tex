...
The main design constraints of Accelera Pipe are graph correctness and safe memory management. 
The pipeline must remain a valid DAG because execution depends on topological sorting, 
so node connections are restricted according to their semantic roles. For example, 
prediction nodes must follow model nodes, metric nodes must follow prediction or merge nodes, 
and merge nodes combine prediction outputs. In addition, the system manages memory by tracking how many downstream nodes 
still need each intermediate output. Once an output is no longer needed, it can be released, reducing memory usage without 
changing the final pipeline result.
