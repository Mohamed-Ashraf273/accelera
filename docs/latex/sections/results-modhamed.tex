...
\begin{table}[H]
\centering
\caption{Windows direct example-script verification runs}
\label{tab:direct-example-runs-windows}
\resizebox{\textwidth}{!}{
\begin{tabular}{llrrrrl}
\toprule
Example & Run/cache state & Baseline time (s) & Accelera/C path time (s) &
Compile/link time (s) & Process wall time (s) & Validation \\
\midrule
\modulepath{accpipe\_demo.py} & normal & 1.75 / 1.78 & \bestcell 1.03 &
-- & 6.53 & same test accuracy, 0.9260 \\
\modulepath{parallel\_accpipe.py} & isolated cache, run 1 & 6.66 &
\bestcell 6.00 & included & 14.37 & outputs match \\
\modulepath{parallel\_accpipe.py} & isolated cache, run 2 & 6.58 &
\bestcell 0.09 & cached & 8.33 & outputs match \\
\modulepath{code\_parallelizer\_demo.py} & isolated cache, run 1 & 5.59 &
\bestcell 1.04 & 0.87 & 8.74 & outputs match \\
\modulepath{code\_parallelizer\_demo.py} & isolated cache, run 2 & 5.61 &
\bestcell 1.05 & 1.04 & 9.00 & outputs match \\
\modulepath{save\_load.py} & normal & 0.57 & \bestcell 0.51 / 0.53 &
-- & 2.71 & same accuracy, 0.268 \\
\bottomrule
\end{tabular}}
\end{table}

Table~\ref{tab:direct-example-runs-windows} reports the same Makefile example
set on Windows. The examples were run one by one in Makefile order. The
