...
PYBIND11_MODULE(graph, m)
{
    ...
    py::class_<Graph>(m, "Graph")
        ...
        .def("add_node", &Graph::add_node, py::arg("type"), py::arg("name"), py::arg("py_func"),
             "Add a node to the graph")

        .def("split", &Graph::split, py::arg("branch_name"), py::arg("branch_objects"),
             py::arg("node_types"), py::arg("node_names"), "Split the graph into branches")

        .def("execute", &Graph::execute, py::arg("X"), py::arg("y"),
             "Execute graph with inputs and return predictions")

        .def("mergeBranches", &Graph::mergeBranches, py::arg("merge_name"), py::arg("merge_func"),
             "Merge all branches back to sequential mode")

        .def("enableParallelExecution", &Graph::enableParallelExecution, py::arg("enable") = true,
             "Enable or disable parallel execution")

        .def("setMulticoreThreshold", &Graph::setMulticoreThreshold, py::arg("threshold"),
    ...
    // Utility functions
    m.def("serialize_graph", &serialize_graph, py::arg("graph"), py::arg("filepath"),
          "Serialize a graph structure to XML file");
}
