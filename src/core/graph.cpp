...
void Graph::split(const std::string& branch_name, const std::vector<py::object>& branch_objects,
...
std::vector<py::object> Graph::execute(py::object X, py::object y)
{
    ...
    for (const auto& leaf : final_leaves)
    {
        if (!leaf->dirty)
        {
            ...
            {
                log_warning("Error collecting results from node '" + leaf->name + "': " + e.what());
            }
        }
    }
...
const std::vector<Node::Ptr>& Graph::getNodes() const
{
    return m_nodes;
}
...
{
    if (m_nodes.empty()) throw std::runtime_error("No nodes in graph");
