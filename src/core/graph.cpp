...
Node::Ptr Graph::add_node(NodeType type, const std::string &name, py::object py_func) {
...
void Graph::split(const std::string &branch_name,
    ...
    for (size_t leaf_idx = 0; leaf_idx < leaves.size(); ++leaf_idx) {
        ...
        for (size_t branch_idx = 0; branch_idx < branch_objects.size(); ++branch_idx) {
            ...
            try {
                ...
                for (size_t list_idx = 0; list_idx < node_list.size(); ++list_idx) {
                    ...
                    Node::Ptr branchNode = NodeFactory::createNode(nodeType, uniqueName, node_obj);
                ...
                if (node_types[branch_idx] == "INPUT") {
                    ...
                    throw std::runtime_error("Unknown node type: " + node_types[branch_idx]);
                }
                ...
                if (leaves.size() == 1) {
                    ...
                    uniqueName = node_names[branch_idx] + "_leaf_" + std::to_string(leaf_idx);
                }

                Node::Ptr branchNode
                    = NodeFactory::createNode(nodeType, uniqueName, branch_objects[branch_idx]);
        ...
        if (!leaf->dirty) {
            try {
                if (leaf->type == NodeType::PREPROCESS) {
                    ...
                    py::object result = py::make_tuple(leaf_node->getX(), leaf_node->getY());
                    if (!result.is_none()) {
                        predictions.push_back(result);
                    }
                } else {
...
void Graph::enableParallelExecution(bool enable) { m_parallel_enabled = enable; }

void Graph::setMulticoreThreshold(size_t threshold) { m_multicore_threshold = threshold; }
...
std::vector<py::object> Graph::getPreprocessingFunctions(Node::Ptr node) const {
    ...
    while (node) {
        ...
        if (node->type == NodeType::PREPROCESS && !py_func.is_none()) {
            m_preprocessing_functions.insert(m_preprocessing_functions.begin(), py_func);
        }
...
void Graph::mergeBranches(const std::string &merge_name, py::object merge_func) {
    if (!m_is_branched) {
...
void Graph::executeNodesInParallel(const std::vector<Node::Ptr> &nodes) {
    ...
    if (!cpu_nodes.empty()) {
        for (size_t i = 0; i < cpu_nodes.size(); ++i) {
            ...
            futures.emplace_back(std::async(std::launch::async, [node, &exceptions, i]() {
    ...
    if (!gpu_nodes.empty()) {
        futures.emplace_back(std::async(
            std::launch::async, [gpu_nodes, &exceptions, cpu_count = cpu_nodes.size()]() {
                py::gil_scoped_acquire acquire; // Acquire GIL for the entire GPU sequence
                try {
