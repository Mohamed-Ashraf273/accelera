    ...
    : Node(NodeType::MODEL, name, py_func)
{
    ...
    {
        throw std::runtime_error("Model node '" + name + "' must have a 'fit' method");
    }
}
...
void ModelNode::execute()
{
    try
    {
        ...
        {
            throw std::runtime_error("Model node '" + name + "' requires a valid preprocess node");
        }
        ...
        {
            throw std::runtime_error("Model node '" + name + "' received None inputs (X or y)");
        }

        if (!py::hasattr(X, "shape") || !py::hasattr(y, "shape"))
        {
            throw std::runtime_error("Model node '" + name + "' inputs must be array-like objects");
        }

        py::object model_instance = py::module::import("copy").attr("deepcopy")(py_func);
        ...
        else
        {
            ...
            {
                throw std::runtime_error("Python error in model fitting: " + std::string(e.what()));
            }
    ...
    {
        throw std::runtime_error("ModelNode: Error during execution: " + std::string(e.what()));
    }
}
