...
void PredictNode::execute()
{
    try
    {
        ...
        {
            throw std::runtime_error("Predict node '" + name + "' requires a valid model node");
        }
        ...
        {
            throw std::runtime_error("Predict node '" + name + "' requires a valid fitted model");
        }
        ...
        if (getGraph())
        {
            ...
            for (const auto& preprocess_func : preprocess_functions)
            {
                if (!preprocess_func.is_none())
                {
                    ...
                    {
                        throw std::runtime_error("Error applying preprocessing to test data: " +
                                                 std::string(e.what()));
                    }
                }
            }
        ...
        if (py::isinstance<py::array>(preprocessed_test_data))
        {
            ...
            {
                throw std::runtime_error("PredictNode: Test data must be at least 1-dimensional");
            }
        }
        ...
        try
        {
            ...
            {
                predictions = fitted_model.attr("predict_proba")(preprocessed_test_data);
            }
            else
            {
        ...
        {
            throw std::runtime_error("Python error in prediction: " + std::string(e.what()));
        }
        setData(predictions);
    }
    catch (const std::exception& e)
    {
        throw std::runtime_error("Error in PredictNode::execute(): " + std::string(e.what()));
    }
}
