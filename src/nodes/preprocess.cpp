...
std::shared_ptr<InputNode> PreprocessNode::getData() const
{
    return m_input;
}
...
void PreprocessNode::execute()
{
    try
    {
        ...
        if (!input)
        {
            throw std::runtime_error("Preprocess node '" + name + "' requires a valid input");
        }
        ...
        if (should_create_new_data)
        {
            ...
            try
            {
                if (py::hasattr(py_func, "fit"))
                {
                    ...
                    transformer_instance = py::module::import("copy").attr("deepcopy")(py_func);
                    try
                    {
            ...
            {
                throw std::runtime_error("Python error in preprocessing: " + std::string(e.what()));
            }
    ...
    {
        throw std::runtime_error("Error in preprocess node '" + name + "': " + e.what());
    }
}
